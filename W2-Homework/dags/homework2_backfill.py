...
with DAG(
    dag_id="homework2_nont_taxi_backfill_v1",
    schedule="@monthly",
    start_date=pendulum.datetime(2019, 1, 1,tz="America/Los_Angeles"),
    catchup=False,
    max_active_runs=1,
    params={
        # "year": Param(
        ...
        download_file = BashOperator(
            ...
            FILE="{{ params.taxi }}_tripdata_${YEAR}-${MONTH}.csv.gz"
            URL="${BASE_URL}${FILE}"
        ...
        upload_file = LocalFilesystemToGCSOperator(
            ...
            dst="{{ params.taxi }}_tripdata_{{data_interval_start.strftime('%Y')}}-{{data_interval_start.strftime('%m')}}.csv",
        ...
        create_ext_table_green_taxi = BigQueryInsertJobOperator(
            ...
            configuration={
                "query": {
                    "query": f"""
                        CREATE OR REPLACE EXTERNAL TABLE `{GCP_PROJECT_ID}.{GCP_DATASET_ID}.{{{{ params.taxi }}}}_tripdata_{{{{ data_interval_start.strftime('%Y') }}}}_{{{{ data_interval_start.strftime('%m') }}}}_ext`
                        (
                        ...
                        OPTIONS (
                        format = 'CSV',
                        uris = ['gs://{GCS_BUCKET_NAME}/{{{{ params.taxi }}}}_tripdata_{{{{ data_interval_start.strftime('%Y') }}}}-{{{{ data_interval_start.strftime('%m') }}}}.csv'],
                        skip_leading_rows = 1,
        ...
        create_int_table_green_taxi = BigQueryInsertJobOperator(
            ...
            configuration={
                "query": {
                    "query": f"""
                        CREATE OR REPLACE TABLE `{GCP_PROJECT_ID}.{GCP_DATASET_ID}.{{{{ params.taxi }}}}_tripdata_{{{{ data_interval_start.strftime('%Y') }}}}_{{{{ data_interval_start.strftime('%m') }}}}`
                        AS
                        SELECT
                            MD5(CONCAT(
                            ...
                            "{{{{ params.taxi }}}}_tripdata_{{{{ data_interval_start.strftime('%Y') }}}}-{{{{ data_interval_start.strftime('%m') }}}}.csv" AS filename,
                            *
                        FROM `{GCP_PROJECT_ID}.{GCP_DATASET_ID}.{{{{ params.taxi }}}}_tripdata_{{{{ data_interval_start.strftime('%Y') }}}}_{{{{ data_interval_start.strftime('%m') }}}}_ext`;
                        """,
                    "useLegacySql": False,
                }
            },     
            ...
            configuration={
                "query": {
                    "query": f"""
                        ...
                        USING `{GCP_PROJECT_ID}.{GCP_DATASET_ID}.{{{{ params.taxi }}}}_tripdata_{{{{ data_interval_start.strftime('%Y') }}}}_{{{{ data_interval_start.strftime('%m') }}}}` S
